                ...
                for n in range(nbstrokes):
                    ...
                    try:
                        ...
                        while (i < nb_points - 4):
                            r_start=i-3
                            r_end=i+4
                            region = a_stroke[r_start:r_end]
                            features = get_green_point_feature(region, corpuslineY, a_stroke, stroke_length)
                            out = sess.run(self.preds, feed_dict={self.model_x: [features]})
                            #print('Scanning region %d to %d ',)
                            if (out[0] == 1): #Found one green point
                                end=i
                                seg_index+=1
                                cut_stroke=a_stroke[start:end+1]
                                #if(len(cut_stroke)>7):
                                gname = sampleid + "_" + strokename + "_" + str(seg_index) + "_" + str(
                                    start) + "_" + str(
                                    end)
                                ...
                                #i += 7
                            i += 7
                        #all points scan of a stroke complete, make the last piece green to red
                        if(start<nb_points-1):
                            end=nb_points-1 #from last green point to red point
                            seg_index+=1
                            cut_stroke=a_stroke[start:end+1]
                            gname = sampleid + "_" + strokename + "_" + str(seg_index) +"_"+ str(start) + "_" + str(
                                end)
                            g = f2.create_group(gname)
                            g.attrs['SampleID'] = sampleid
                            g.attrs['Stroke_Length'] = len(cut_stroke)
...
def predict_points_of_sample(nb_features,sampleid,hdffile):
    ...
    for n in range(nbstrokes):
        strokename="S"+str(n)
        print("processing stroke %s"%strokename)
        ...
        while(i<nb_points-4):
            ...
            region=a_stroke[r_start:r_end]
            features=get_green_point_feature(region,corpuslineY,a_stroke,stroke_length)
            print("\tScanning from %d to %d"%(r_start,r_end))
            out=net.predict(features)
            if(out[0]==1):#found one green point
                print("\t\tFound Green at %d for Stroke %s"%(i,strokename))
                a_stroke[i][2]=2
                #i+=10
            i+=7
        all_strokes.append(a_stroke)
...
# predict_points_of_sample(nbfeat,"Aditya13_AmdAnI.txt",hdfin)
